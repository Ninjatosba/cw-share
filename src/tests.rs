...
mod tests {
    ...
    use cosmwasm_std::{
        from_binary, to_binary, Addr, BankMsg, Coin, Decimal, Decimal256, Empty, MessageInfo,
        SubMsg, Uint128, WasmMsg,
    };

    use crate::contract::{execute, get_decimals, instantiate, query, execute_bond};
    use crate::msg::{
...
pub fn test_bond() {
    ...
    //first bonding
    let info: MessageInfo = mock_info("staker1", &[Coin{
        denom: "staked".to_string(),
        amount: Uint128::from(1000000),
    }]);
    let res = execute_bond(deps.as_mut(), env.clone(), info.clone(), ).unwrap();
    assert_eq!(0, res.messages.len());
